...
/**
 * Build permission overwrites for a new court request channel.
 * Inherits all overwrites from the parent category, then adds/merges
 * the reporter's personal overwrite and the bot's overwrite on top.
 * This ensures every role that can see the category can also see the ticket.
 */
function buildTicketOverwrites(category, reporterId, botId) {
  // Copy all existing category overwrites
  ...
  // Add (or override) the reporter's personal overwrite
  const existingReporterIdx = overwrites.findIndex(o => o.id === reporterId);
  ...
  if (existingReporterIdx !== -1) {
    overwrites[existingReporterIdx] = reporterOverwrite;
  } else {
    overwrites.push(reporterOverwrite);
  }
...
module.exports = {
  ...
  async modalHandler(interaction) {
    ...
    if (interaction.customId.startsWith("court_request_modal_")) {
      ...
      // access can see the ticket, while still granting the reporter personal access.
